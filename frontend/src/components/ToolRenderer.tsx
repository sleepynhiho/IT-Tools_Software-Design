...
import CustomPluginUIs, {
  CustomPluginUIProps,
} from "./plugins/CustomPluginUIs";
...
} from "../data/pluginList"; // Adjust path
...
const ToolRenderer: React.FC = () => {
        ...
        const func = new Function(
          ...
          `
                try { 
                    ...
                    console.error('Condition evaluation error:', '${conditionStr.replace(
                      /'/g,
                      "\\'"
                    )}', e); 
                    return false; 
  ...
  const processRequest = useCallback(
    async (currentFormData: Record<string, any>) => {
      ...
      try {
        ...
        setResultData({
          success: false,
          [errorKey]: error.message || "Network error.",
        });
      } finally {
  ...
  useEffect(() => {
    ...
    if (!Array.isArray(allPluginMetadata)) {
    ...
    console.log(
      `Finding metadata for id: ${toolIdFromUrl} in list of ${allPluginMetadata.length}`
    );
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        "Effect Triggering initial process request with formData:",
        formData
      );
                            ...
                            .map((output: OutputField) => {
                              ...
                              const shouldRender =
                                (resultData?.success === false &&
                                  ...
                                  outputValue !== null) || // Show error field if error state and value exists
                                (resultData?.success === true &&
                                  outputValue !== undefined &&
                                  outputValue !== null); // Show normal field if success state and value exists
  ...
  switch (field.type) {
    ...
    case "switch":
      return (
        <Box>
          {" "}
          {/* Wrap in Box to contain helper text */}
          <Box
            sx={{
              display: "flex",
